...
const mockQuestionSchema = z.object({
    question: z.string().describe("The next single interview question to ask the candidate.")
});

const answerEvaluationSchema = z.object({
    score: z.number().min(0).max(100).describe("Score for this answer (0-100) based on accuracy, depth, and relevance."),
    feedback: z.string().describe("Constructive, short feedback highlighting strengths and key improvement areas.")
});

const sessionFeedbackSchema = z.object({
    overallScore: z.number().min(0).max(100).describe("Overall average score across all answers (0-100)."),
    feedbackReport: z.string().describe("A detailed summary review explaining how they did and next steps for learning.")
});

// ── 2. RESUME INTELLIGENCE SCHEMA ──
const resumeAnalysisSchema = z.object({
    atsScore: z.number().min(0).max(100).describe("ATS score from 0 to 100."),
    formattingProblems: z.array(z.string()).describe("Formatting, section titles, layout issues."),
    missingKeywords: z.array(z.string()).describe("Important keywords or skills missing from the resume relative to the job description."),
    weakBulletPoints: z.array(z.object({
        before: z.string().describe("Original bullet point"),
        after: z.string().describe("Action-oriented, metrics-focused replacement bullet point")
    })).describe("Original bullet points and suggestions to rewrite them with metrics and impact."),
    experienceQualityReport: z.string().describe("General overview of formatting and content quality.")
});

// ── 3. LINKEDIN ANALYSIS SCHEMA ──
const linkedinAnalysisSchema = z.object({
    headlineSuggestions: z.array(z.string()).describe("3 search-optimized headlines incorporating target keywords."),
    aboutSuggestions: z.string().describe("A professional, high-impact Summary / About section for the profile."),
    experienceSuggestions: z.array(z.object({
        company: z.string(),
        role: z.string(),
        suggestions: z.string().describe("Specific tips to enhance this specific experience section.")
    }))
});
...
const adaptiveRoadmapSchema = z.object({
    weeks: z.array(z.object({
        weekNumber: z.number(),
        focus: z.string().describe("The core technical or conceptual focus of this week."),
        tasks: z.array(z.string()).describe("Actions, labs, coding practices, or reading tasks.")
    }))
});

// ── 5. CAREER ASSISTANT CHAT SCHEMA ──
const careerAssistantSchema = z.object({
    response: z.string().describe("Comprehensive career advice. Reference the user's details if relevant.")
});
 ...
 */
async function generateMockQuestion({ role, experienceLevel, interviewType, chatHistory }) {
    let historyPrompt = "";
    if (chatHistory && chatHistory.length > 0) {
        historyPrompt = chatHistory.map(msg => `${msg.role === 'interviewer' ? 'Interviewer' : 'Candidate'}: ${msg.content}`).join("\n");
    }
...
async function generateSessionSummary({ role, interviewType, chatHistory }) {
    const historyText = chatHistory
        .map(h => `${h.role === 'interviewer' ? 'Q' : 'A'}: ${h.content} (Eval Score: ${h.evaluation?.score || 'N/A'})`)
        .join("\n");
 ...
 */
async function generateAdaptiveRoadmap({ targetRole, currentSkills, missingSkills, availableWeeks }) {
 ...
 */
async function chatCareerAssistant({ message, chatHistory, resumeText, targetRole, skillGaps }) {
