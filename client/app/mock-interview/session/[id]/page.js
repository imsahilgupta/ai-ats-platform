...
export default function MockInterviewSession() {
      ...
      if (SpeechRecognition) {
        ...
        rec.onerror = (e) => {
          ...
          if (e.error === 'not-allowed') {
            showToast("Microphone access was denied. Please allow microhone permissions.", "error");
  ...
  const toggleListening = () => {
    if (!recognitionRef.current) {
      showToast("Speech Recognition is not supported in this browser. Please use text input or Chrome/Edge.", "warning");
      return;
    }
    ...
    if (isListening) {
      ...
      try {
        ...
        setIsListening(true);
        showToast("Listening... Speak clearly into your microphone.", "success");
      } catch (err) {
  ...
  const activeQuestion = lastInterviewerMessage ? lastInterviewerMessage.content : "Starting simulation...";
      ...
      <div className="home-page" style={{ maxWidth: '1000px', margin: '0 auto', padding: '2rem 1rem' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <div>
            ...
            <p style={{ margin: '0.2rem 0 0 0', fontSize: '0.9rem', color: '#aaa' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '2rem' }}>
          {/* Interviewer Box */}
          <div className="interview-card" style={{ padding: '2rem', minHeight: '180px', display: 'flex', flexDirection: 'column', justifyContent: 'center', position: 'relative' }}>
            <div style={{ position: 'absolute', top: '1rem', left: '2rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span style={{ width: '8px', height: '8px', borderRadius: '50%', background: submitting ? '#f5a623' : '#3fb950', display: 'inline-block', animation: submitting ? 'pulse 1.5s infinite' : 'none' }} />
              <span style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#888', uppercase: true }}>
            ...
            <p style={{
              ...
              {submitting ? "Analyzing your answer and preparing the next question. Please wait..." : activeQuestion}
            </p>
          </div>

          {/* User Response Box */}
          <form onSubmit={handleSubmit} className="interview-card" style={{ overflow: 'hidden' }}>
            <div className="interview-card__body" style={{ flexDirection: 'column', gap: '1.5rem', padding: '2rem' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '0.4rem 1rem',
                    fontSize: '0.85rem',
                    borderColor: isListening ? '#ff4d4f' : 'rgba(255,255,255,0.1)',
                    background: isListening ? 'rgba(255, 77, 79, 0.2)' : 'rgba(255,255,255,0.03)',
                    color: isListening ? '#ff4d4f' : '#fff'
                  }}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ animation: isListening ? 'pulse 1s infinite' : 'none' }}>
            ...
            <div className="interview-card__footer" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
              <span className="footer-info">Make sure to address the core problem using STAR framework.</span>
              <button
                type="submit"
              ...
              >
                {submitting ? (
                  ...
                  <>
                    Submit Answer
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ marginLeft: '0.5rem' }}>
